...
//   1) La firma ya no asume que las propiedades son siempre
//      [id, status, amount_in_cents]: ahora lee `signature.properties`
//      del propio evento (así lo define Wompi) y arma el checksum
//      dinámicamente, tal como indica su documentación. Si cambias
//      la configuración de eventos en el dashboard de Wompi, esto
//      sigue funcionando sin tocar código.
//   2) Si WOMPI_EVENTS_SECRET no está configurado, antes el checksum
//      simplemente nunca iba a coincidir (fallaba en silencio como
//      "firma_invalida" para siempre). Ahora se detecta y avisa
//      claramente con error 500 "webhook_no_configurado".
//
...
// Lee un valor anidado del body del evento a partir de una ruta tipo
// "transaction.id" (así es como Wompi lista sus `signature.properties`).
function leerRuta(obj: unknown, ruta: string) {
  return ruta.split(".").reduce((acc: any, key) => (acc == null ? acc : acc[key]), obj);
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!secret) return json({ error: "webhook_no_configurado" }, 500);

    // Wompi indica en cada evento qué propiedades concatenar para el
    // checksum (por defecto suele ser transaction.id, transaction.status,
    // transaction.amount_in_cents, pero puede variar según su config).
    const props = Array.isArray(properties) && properties.length
      ? properties
      : ["transaction.id", "transaction.status", "transaction.amount_in_cents"];
    const valores = props.map((p) => leerRuta(body.data, p)).join("");
    const calc = await sha256hex(`${valores}${timestamp}${secret}`);
